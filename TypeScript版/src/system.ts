...
export let target = Math.floor(g.game.random.generate() * 4);
...
export function createInsect(scene: Scene, colorIdx: number, sizeIdx: number, hornSizeIdx: number) {
...
function moveInsect(insect: FilledRect, flg: boolean, LorR: number): [boolean, number] {
...
export function updateStatus(leg1: FilledRect) {
  count--;
...
export function missIndicate(scene: Scene, x: number, y: number) {
